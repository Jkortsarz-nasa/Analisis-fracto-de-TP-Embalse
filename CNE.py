...
def Deuterium (ci, time):
    T=273 + 250 #K -Temperature inlet end
    L=6.171 #m -largo del tubo
    ...
    xCNE = np.array([2,	4,	5,	5.6])
    yCNE = np.array([2.36,	3.12,	3.69,	4.36])

    #plt.plot(xCNE, yCNE, 'b^', label='N1K05 Inlet')

    # xCNE = np.array([2,4,5,5.6])
    # yCNE = np.array([28.8,23.5,22.9,29])
    
    p0=[0.2,0.4,0.1]

    def modelo(x0, A, B, C):
        return RJDIM.diferencias(x0, A, B, C, ci=ci, time = time)

    popt, pcov = curve_fit(RJDIM.diferencias, xCNE, yCNE, p0=p0,method='lm')

    M = RJDIM.diferencias(x, popt[0], popt[1], popt[2], ci, time)
    ...
    y = RJDIM.diferencias(x, popt[0], popt[1], popt[2], ci, time)
    t = temp(x)
