...
def diferencias(x, A, B, C):
    ...
    while j<=time:
        ...
        b[1:-1]=dt*cg[1:-1]
        b[-1]=dt*(cg[-1] + (1/(masa*(a2*np.sqrt((j)*dt)+b2))))
        C=np.dot(A,C)+0.1*b
        j=j+dt
...
ax1.plot(np.array([1.98,	3.96,	4.98]), np.array([1.48,	8.96,11.78]),'b^', color='black', label='D11')
#ax1.plot(np.array([1.98,	3.96,	4.93,	5.53]), np.array([2.48,	12.73,	12.25,	12.94]),'b^', color='red', label='data 2')
ax1.legend()
ax1.grid(True)
