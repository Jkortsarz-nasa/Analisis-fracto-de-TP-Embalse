...
def diferencias(x, A, B, C):
    ...
    time= 12.34589 #years of reactor operation conditions exposure 
    ...
    K=np.eye(nodos_x,nodos_x)
    for i in range(0,nodos_x):
        K[i,i]=(1-2*r_temp(i*dx))
...
y = diferencias(x,0.18511348,  -0.70806712, -50.3452297)
t = temp(x)
...
#ax1.plot(np.array([1.98,	3.96,	4.98]), np.array([1.48,	8.96,11.78]),'b^', color='black', label='data 1')
ax1.plot(np.array([1.98,	3.96,	4.93,	5.53]), np.array([2.48,	12.73,	12.25,	12.94]),'b^', color='red', label='F06')
ax1.legend()
ax1.grid(True)
